...
THREE.OAVLoader.prototype = {
		...
		for ( var i = 0, l = lines.length; i < l; ++ i ) {
			...
			if ( coord.length == 11) {
				...
				if ( e.length ) {
				    ...
				    atoms.push( [ x, y, z, [144, 144, 144], e[3], e[1], parseInt(e[2]) ] );
                }
	...
	createModel: function ( json, callback ) {
		...
		for ( i = 0, l = atoms.length; i < l; i ++ ) {
			...
			var z = atom[ 2 ];
			var chainId = atom[5];
			var sequenceId = atom[6];

			verticesAtoms.push( x, y, z );
			positionBonds.push( chainId, sequenceId );
...
/*
		...
		callback( geometryAtoms, geometryBonds, positionBonds, json );
